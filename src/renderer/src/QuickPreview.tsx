import type { Item, ItemType } from '../../preload/types'
...
function Content({ item, src }: { item: Item; src: string }): React.JSX.Element {
  ...
  if (item.type === 'audio') {
    return (
      <div onClick={stop} style={{ textAlign: 'center', color: '#fff' }}>
        <div style={{ fontSize: 64, marginBottom: 16 }}>{TYPE_GLYPH.audio}</div>
  ...
  return (
    <div onClick={stop} style={{ textAlign: 'center', color: '#fff' }}>
      <div style={{ fontSize: 80, marginBottom: 16 }}>{TYPE_GLYPH[item.type]}</div>
